...
class TabSTARModel(nn.Module):
    ...
    It combines:
    1. A Text Encoder (E5-small) for semantic understanding.
    ...
    def __init__(self, d_model: int = 384, n_layers: int = 6, n_heads: int = 6):
        ...
        self.tabular_encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)
    ...
    def freeze_text_encoder(self):
        """Freezes the E5 text encoder parameters."""
        for param in self.text_encoder.parameters():
            param.requires_grad = False

    def unfreeze_text_encoder(self):
        """Unfreezes the E5 text encoder parameters."""
        for param in self.text_encoder.parameters():
    ...
    def forward(
        ...
        Args:
            feature_input_ids: (Batch, Num_Features, Seq_Len)
            feature_attention_mask: (Batch, Num_Features, Seq_Len)
            feature_num_values: (Batch, Num_Features)
            target_token_ids: (Num_Classes, Seq_Len_Target)
            target_attention_mask: (Num_Classes, Seq_Len_Target)
        ...
        target_outputs = self.text_encoder(input_ids=target_token_ids, attention_mask=target_attention_mask)
        target_embeddings = self._mean_pooling(target_outputs, target_attention_mask) # (C, D)
        target_embeddings = target_embeddings.unsqueeze(0).expand(B, -1, -1) # (B, C, D)
        
        # Numerical values for targets are always 0
        ...
        # Flatten to process all features of all samples in one go
        flat_feature_ids = feature_input_ids.view(B * M, L)
        flat_feature_mask = feature_attention_mask.view(B * M, L)
        
        feature_outputs = self.text_encoder(input_ids=flat_feature_ids, attention_mask=flat_feature_mask)
        feature_embeddings = self._mean_pooling(feature_outputs, flat_feature_mask) # (B*M, D)
        feature_embeddings = feature_embeddings.view(B, M, D)
        
        # Fuse features with their magnitudes (clipped z-scores)
        fused_feature_embeddings = self.numerical_fusion(feature_embeddings, feature_num_values) # (B, M, D)
        
        # --- Step 3: Interaction Encoding ---
        # Sequence: [Fused Target Tokens] + [Fused Feature Tokens]
        full_sequence = torch.cat([fused_target_embeddings, fused_feature_embeddings], dim=1) # (B, C+M, D)
        encoded = self.tabular_encoder(full_sequence) # (B, C+M, D)
        
        # --- Step 4: Prediction ---
        # Extract the contextualized target tokens
        contextualized_targets = encoded[:, :C, :] # (B, C, D)
        
        if task_type == 'regression':
            # For regression, C=1. Shared head projects to a single value.
            scores = self.reg_head(contextualized_targets).squeeze(-1) # (B, 1)
        else:
            # For classification, shared head produces a logit for each class token.
